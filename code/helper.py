...
def load_constants():
    ...
    dir = '../data/constants'
    cities = pickle.load(open('{}/cities.p'.format(dir),'rb'))

    if MODE == 'Shooter':
        with open('{}/male_first_names.json'.format(dir),'rb') as outfile:
            male_first_names = set(json.load(outfile))
        with open('{}/female_first_names.json'.format(dir),'rb') as outfile:
            female_first_names = set(json.load(outfile))
        with open('{}/last_names.json'.format(dir),'rb') as outfile:
            last_names = set(json.load(outfile))
        with open('{}/train_names.json'.format(dir),'rb') as outfile:
            train_names = set(json.load(outfile))
        with open('{}/number_as_words.json'.format(dir),'rb') as outfile:
            number_as_words = set(json.load(outfile))
        with open('{}/word_ordinals.json'.format(dir),'rb') as outfile:
    ...
    #other_features = []
        ...
        with open('{}/adulterant_part.p'.format(dir),'rb') as outfile:
            adulterant_parts = set(pickle.load(outfile))
        with open('{}/adulterants.p'.format(dir),'rb') as outfile:
            adulterants = set(pickle.load(outfile))
        with open('{}/foods.p'.format(dir),'rb') as outfile:
            foods = pickle.load(outfile)
        with open('{}/number_as_words.json'.format(dir),'rb') as outfile:
            number_as_words = set(json.load(outfile))
        with open('{}/word_ordinals.json'.format(dir),'rb') as outfile:
            word_ordinals = set(json.load(outfile))
        with open('{}/country_names.p'.format(dir), 'rb') as outfile:
